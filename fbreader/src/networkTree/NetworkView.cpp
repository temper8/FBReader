...
NetworkView::NetworkView() : myUpdateChildren(true), myUpdateAccountDependents(false) {
	//myDialog = ZLDialogManager::Instance().createTreeDialog(ZLResourceKey("NetworkView"));
        //TODO fix hack:
	...
	AppLog("createTreeDialog");
	//myDialog->rootNode().registerTreeAction(new AddCatalogAction(this));
	AppLog("registerTreeAction");
}

void NetworkView::showDialog() {
	if (myDialog.isNull()) myDialog = ZLDialogManager::Instance().createTreeDialog(ZLResourceKey("NetworkView"));
	//myDialog->rootNode().registerTreeAction(new AddCatalogAction(this));
	AppLog("showDialog");
	makeUpToDate();
	AppLog("makeUpToDate");
